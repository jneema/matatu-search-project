...
const StepBulkDestinations = ({ formData, onDestChange, onAddDest, onRemoveDest, errors }) => (
  <div className="space-y-4">
    <div className="mb-6">
      <h2 className="text-2xl font-extrabold text-gray-900">Bulk destinations</h2>
      ...
      return (
        <div key={i} className="border border-gray-200 rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 bg-gray-900">
            <span className="text-xs font-bold text-white uppercase tracking-wider">Destination {i + 1}</span>
            {formData.destinations.length > 1 && (
              <button onClick={() => onRemoveDest(i)} className="text-white/50 hover:text-white">
          ...
          <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
            <Field label="Road *" error={e.road}>
              <select value={dest.road} onChange={(e) => onDestChange(i, "road", e.target.value)}
            ...
            <Field label="Destination name *" error={e.destination}>
              <input type="text" value={dest.destination}
                onChange={(e) => onDestChange(i, "destination", e.target.value)}
                placeholder="e.g., Zimmerman" className={inputClass(errors[i]?.destination)} />
            </Field>
            <Field label="Departure *" error={e.departure}>
              <input type="text" value={dest.departure}
                onChange={(e) => onDestChange(i, "departure", e.target.value)}
                placeholder="e.g., CBD" className={inputClass(errors[i]?.departure)} />
            </Field>
            <Field label="Distance">
              <input type="text" value={dest.distance}
