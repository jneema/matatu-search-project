          ...
          <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
            <Field label="Destination ID *" error={e.destinationId}>
              <input
                ...
                onChange={(ev) =>
                  onMatatuChange(i, "destinationId", ev.target.value)
                }
                placeholder="e.g., 3"
                className={inputClass(e.destinationId)}
              />
            </Field>
            <Field label="Matatu name">
              <input
                ...
                value={m.matatuName}
                onChange={(ev) =>
                  onMatatuChange(i, "matatuName", ev.target.value)
                }
            ...
            <Field label="CBD stage *" error={e.cbdStage}>
              <input
                ...
                value={m.cbdStage}
                onChange={(ev) =>
                  onMatatuChange(i, "cbdStage", ev.target.value)
                }
                placeholder="e.g., Railways"
                className={inputClass(e.cbdStage)}
              />
            </Field>
            <Field label="Estate stage *" error={e.estateStage}>
              <input
                ...
                value={m.estateStage}
                onChange={(ev) =>
                  onMatatuChange(i, "estateStage", ev.target.value)
                }
                placeholder="e.g., TRM"
                className={inputClass(e.estateStage)}
              />
            </Field>
            <Field label="Peak fare inbound *" error={e.peakFareInbound}>
              <input
                type="number"
                value={m.peakFareInbound}
                onChange={(ev) =>
                  onMatatuChange(i, "peakFareInbound", ev.target.value)
                }
                placeholder="e.g., 120"
                className={inputClass(e.peakFareInbound)}
              />
            </Field>
            <Field label="Peak fare outbound *" error={e.peakFareOutbound}>
              <input
                type="number"
                value={m.peakFareOutbound}
                onChange={(ev) =>
                  onMatatuChange(i, "peakFareOutbound", ev.target.value)
                }
                placeholder="e.g., 100"
                className={inputClass(e.peakFareOutbound)}
              />
            </Field>
            <Field label="Off-peak fare *" error={e.offPeakFare}>
              <input
                ...
                value={m.offPeakFare}
                onChange={(ev) =>
                  onMatatuChange(i, "offPeakFare", ev.target.value)
                }
                placeholder="e.g., 80"
                className={inputClass(e.offPeakFare)}
              />
            </Field>
              ...
              <Field label="Payment methods *" error={e.payment}>
                <div className="grid grid-cols-3 gap-2 mt-0.5">
                  {paymentOptions.map((opt) => (
                    <label
                      key={opt}
                      className={`flex items-center gap-2 p-2.5 border rounded-md cursor-pointer hover:bg-gray-50 ${e.payment ? "border-red-400" : "border-gray-200"}`}
                    >
                      <input
                        type="checkbox"
                        value={opt}
