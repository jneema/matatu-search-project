...
const RoutesView = ({
  ...
  // Normalize destination name to handle "CBD" and "Nairobi CBD" as the same
  const isCBDSelected = selectedDestination?.name.toLowerCase().includes("cbd");

  // Normalize currentLocation to match routesData keys
  const normalizedCurrentLocation = currentLocation
    ? Object.keys(
        routesData[selectedTown?.name]?.[selectedRoad?.name] || {}
      ).find((key) => key.toLowerCase() === currentLocation.toLowerCase()) ||
      currentLocation
    : null;

  // Fetch routes based on whether CBD is selected
  const routes =
    selectedTown && selectedRoad && selectedDestination
      ? isCBDSelected && normalizedCurrentLocation
        ? // If CBD is selected, fetch routes from Nairobi CBD to normalizedCurrentLocation
          routesData[selectedTown.name]?.[selectedRoad.name]?.[
            normalizedCurrentLocation
          ]?.map((route) => ({
            ...route,
            departure: normalizedCurrentLocation, // Swap departure to currentLocation
            destination: "Nairobi CBD", // Swap destination to Nairobi CBD
            dropoffs: [...route.dropoffs].reverse(), // Reverse dropoffs to simulate reverse route
          })) || []
        : // Otherwise, fetch routes from Nairobi CBD to selectedDestination
          routesData[selectedTown.name]?.[selectedRoad.name]?.[
      ...
      <div className="max-w-4xl mx-auto px-3 sm:px-4 py-8 sm:py-16">
        ...
        <div className="mb-8 sm:mb-12">
          <h3 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4 sm:mb-6 text-center px-2">
            {isCBDSelected
              ? `Routes from ${
                  normalizedCurrentLocation || "your location"
                } to Nairobi CBD`
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 space-y-2 sm:space-y-0">
                    <h4 className="text-lg sm:text-xl font-semibold text-gray-800 pr-2">
                      {route.name} from {route.departure} to {route.destination}
                    </h4>
